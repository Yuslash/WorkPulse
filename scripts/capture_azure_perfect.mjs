...
async function capture() {
  ...
  console.log('2. Navigating to Settings to click Azure Pulse card...');
  await page.goto('http://localhost:5173/settings', { waitUntil: 'networkidle' });
  await page.waitForTimeout(1500);

  const azureCard = page.locator('text=Azure Pulse').first();
  if (await azureCard.count() > 0) {
    console.log('Found Azure Pulse card, clicking...');
    await azureCard.click();
    await page.waitForTimeout(1500);
  } else {
    console.log('Setting via localStorage...');
    await page.evaluate(() => {
  ...
  // Also capture warm cream explicitly
  console.log('Capturing Warm Cream explicitly...');
  await page.goto('http://localhost:5173/settings', { waitUntil: 'networkidle' });
  await page.waitForTimeout(1000);
  const warmCard = page.locator('text=Warm Cream').first();
  if (await warmCard.count() > 0) {
    ...
    await page.waitForTimeout(1000);
    for (const dir of PREVIEW_DIRS) {
      await page.screenshot({ path: path.join(dir, 'theme_warm_cream.png') });
    }
  }
